...
for(const [name,historicalCrlf] of [['canonical',false],['historical_crlf',true]]){
 ...
 try{
  ...
  assert.equal(await applyMigrations(db,{phase:'upgrade'}),5);
...
const artifact={
 ...
 profile_definition:'canonical: all reviewed LF SQL; historical_crlf: first 33 reviewed SQL files encoded CRLF, then five new LF migrations. UTC and fixed search_path. Synthetic Auth/Storage only.',
