...
- 必填：`patient_code`（研究编号，中心内唯一即可，如 `0001`）
...
- patient_code：中心内唯一即可（如 `0001`, `0002`）。多中心合并键为 `center_code + patient_code`，无需在编号中重复中心前缀。
...
## Q1：登录遇到问题？
**当前登录方式**：邮箱 + 密码。如忘记密码，请在登录页点击"忘记密码"重置。如仍有问题请联系 china@kidneysphere.com。
...
从本版本开始，系统采用“内部英文编码 + 前台中文展示”双层结构：

- 前台默认显示中文字段名与中文解释。
- 导出默认使用中文列名；高级用户可切换英文编码导出。
- 常见缩写首次出现应展示中文全称（例如：肾供体概况指数 KDPI）。
- 支持中文别名搜索（例如：肌酐、尿蛋白、排斥、BK 病毒）。

详细规范见：`docs/CN_FRIENDLY_UPGRADE_SPEC.md`。
