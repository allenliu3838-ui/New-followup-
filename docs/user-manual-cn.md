...
- 患者编号（patient_code）：`0001`（中心内唯一即可，合并键为 center_code + patient_code）
...
|---|---|---|---|---|
| **患者编号（patient_code）** | ✅ | 中心内唯一即可；多中心合并键为 center_code + patient_code | `0001` | 不同中心使用相同编号且未区分 center_code |
...
系统已新增中文优先的数据层能力（概念字典、缩写词典、中文别名搜索、中文导出映射）。

- 规范文档：`docs/CN_FRIENDLY_UPGRADE_SPEC.md`
- 数据库迁移：`supabase/migrations/0019_cn_friendly_layer.sql`

建议前端默认启用新手模式，隐藏英文 raw code，仅在专业模式显示。
